...
+
サードパーティのIDプロバイダーから、プレーンテキスト形式のIDプロバイダーメタデータが取得されます。このメタデータは、JSON形式で正しくフォーマットされていることを確認するために検証する必要があります。利用できるJSONフォーマッタアプリケーションは多数あります。例えば：https://freeformatter.com/json-escape.html
